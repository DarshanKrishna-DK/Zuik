...
export async function deploy() {
  ...
  // Prepare ABI method call arguments for createApplication
  ...
  const createTxn = algosdk.makeApplicationCreateTxnFromObject({
    ...
    numGlobalInts: 4, // totalTransactions, totalVolume, lastResetRound, isPaused
    numGlobalByteSlices: 1, // owner
    numLocalInts: 6, // agentIsActive, dailySpendingCap, dailySpentAmount, lastDayReset, transactionCount, riskScore
    numLocalByteSlices: 2, // allowedAssets, allowedMethods
    appArgs,
    suggestedParams,
  })
  ...
  // Fund the app account for storage costs
  const appAddress = algosdk.getApplicationAddress(appId)
  const fundTxn = algosdk.makePaymentTxnWithSuggestedParamsFromObject({
    sender: deployer.addr,
    receiver: appAddress,
    amount: BigInt(500_000), // Higher funding for local state storage
    suggestedParams,
  })
  ...
  // Store deployment info for use by other scripts
  ...
  console.log('Guardian Smart Contract deployed successfully! 🛡️')

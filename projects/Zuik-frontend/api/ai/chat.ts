...
async function retryWithBackoff<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.log(`[AiChat] Rate limited, retrying in ${Math.round(finalDelay)}ms (attempt ${attempt + 1}/${maxRetries + 1})`)
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (isRateLimit) {
      console.error('[AiChat] Rate limit exceeded after all retries:', msg)
    ...
    console.error('[AiChat] Request failed:', msg)

...
async function getUpdates(): Promise<TelegramUpdate[]> {
  ...
  try {
    const res = await fetch(
      `https://api.telegram.org/bot${TELEGRAM_TOKEN}/getUpdates?offset=${lastUpdateId + 1}&timeout=5&allowed_updates=["message"]`,
    )
    if (!res.ok) return []
...
async function sendKeyboard(chatId: number, text: string, buttons: { text: string; callback_data?: string }[][]) {
  if (!TELEGRAM_TOKEN) return
...
async function handleStart(chatId: number, firstName?: string) {
  await sendReply(chatId, [
    `Welcome to <b>Zuik</b>${firstName ? `, ${firstName}` : ''}!`,
    '',
    'I am your DeFi automation assistant on Algorand.',
    '',
    '<b>Commands:</b>',
    '/link &lt;wallet_address&gt; - Link your Algorand wallet',
    '/balance - Check your ALGO wallet balance',
    '/price - Get live ALGO/USD price',
    '/workflows - List your saved workflows',
    '/status - Check active workflow schedules',
    '/profit - View execution history and performance',
    '/help - Show this message',
    '',
    'Or just type a message and I will help you with DeFi strategies!',
  ].join('\n'))
}
...
async function handleLink(chatId: number, walletAddress: string) {
  ...
  const { error } = await supabase
    .from('telegram_links')
    .upsert({ wallet_address: walletAddress, telegram_chat_id: String(chatId) }, { onConflict: 'telegram_chat_id' })
  ...
  await sendReply(chatId, `Wallet linked! <code>${shortAddr}</code>\n\nYou will now receive notifications from your Zuik workflows.`)
}
...
async function handlePrice(chatId: number) {
  try {
    const res = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=algorand&vs_currencies=usd&include_24hr_change=true&include_24hr_vol=true&include_market_cap=true')
    if (!res.ok) {
      await sendReply(chatId, 'Could not fetch price data. Try again later.')
      return
    }
    const data = await res.json() as { algorand?: { usd?: number; usd_24h_change?: number; usd_24h_vol?: number; usd_market_cap?: number } }
    ...
    await sendReply(chatId, [
      `<b>ALGO/USD Live Price</b>`,
      '',
      `Price: <b>$${algo.usd.toFixed(4)}</b>`,
      `24h Change: ${arrow} <b>${change >= 0 ? '+' : ''}${change.toFixed(2)}%</b>`,
      `24h Volume: ${vol}`,
      `Market Cap: ${mcap}`,
    ].join('\n'))
  } catch (e) {
    ...
    const res = await fetch(GROQ_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are Zuik, a friendly DeFi automation assistant on Algorand. Keep responses concise (2-3 sentences). Help users with trading strategies, DCA, price alerts, and Algorand DeFi concepts. If they want to build a workflow, tell them to use the Zuik web app.',
          },
...
async function pollTelegram() {
  ...
  for (const update of updates) {
    ...
    if (text === '/start' || text === '/help') {
      ...
      await sendReply(chatId, 'Unknown command. Type /help for available commands.')
    } else {
